...
:   Node("ORB_SLAM3_ROS2")
{
    ...
    m_image_subscriber = this->create_subscription<ImageMsg>(
        ...
        1, std::bind(&MonocularInertialNode::GrabImage, this, _1));

    subImu_ = this->create_subscription<ImuMsg>(
        "bcr_bot/imu", 10000, std::bind(&MonocularInertialNode::GrabImu, this, _1));
...
void MonocularInertialNode::SyncWithImu()
{
    while (rclcpp::ok()) {
        ...
        if (!imgBuf_.empty() && !imuBuf_.empty()) {
            ...
            // Ensure latest IMU data is newer than image timestamp
            ...
            while (!imuBuf_.empty() && Utility::StampToSec(imuBuf_.front()->header.stamp) <= tImage) {
                ...
                // Check for NaNs again just in case
                if (std::isnan(acc.x) || std::isnan(acc.y) || std::isnan(acc.z) ||
                    std::isnan(gyr.x) || std::isnan(gyr.y) || std::isnan(gyr.z)) {
                    RCLCPP_WARN(this->get_logger(), "IMU data with NaNs detected, skipping this measurement");
                    continue;
                }
            ...
            if (vImuMeas.empty()) {
                RCLCPP_WARN(this->get_logger(),
                        "Skipping image at time %.6f because no valid IMU data was found before it.",
                        tImage);
                img_lock.unlock();
                imu_lock.unlock();
                std::this_thread::sleep_for(std::chrono::milliseconds(5));
